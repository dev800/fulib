# the cool way
require Fulib.Cipher.Helpers, as: H

defmodule Fulib.Cipher do
  @moduledoc """
  ...
  defp get_phrase(:keyphrase), do: H.env(:keyphrase)
  defp get_phrase(:ivphrase), do: H.env(:ivphrase)
  ...
  @doc """
    ...
    ```elixir
    "secret"
    |> Fulib.Cipher.encrypt  # "KSHHdx0uyveYGY5PHqLAKw%3D%3D"
    |> Fulib.Cipher.decrypt  # "secret"
    ```
  """
  def encrypt(data, keyphrase \\ nil, ivphrase \\ nil) when is_binary(data) do
    k = (keyphrase || get_phrase(:keyphrase)) |> Fulib.Cipher.Digest.generate_key()
    i = (ivphrase || get_phrase(:ivphrase)) |> Fulib.Cipher.Digest.generate_iv()
  ...
  defp _decrypt(decoded, keyphrase, ivphrase) do
    k = (keyphrase || get_phrase(:keyphrase)) |> Fulib.Cipher.Digest.generate_key()
    i = (ivphrase || get_phrase(:ivphrase)) |> Fulib.Cipher.Digest.generate_iv()
    ...
    try do
      ...
      _ ->
        {:error,
         "Could not decrypt string '#{decoded}'. Maybe it was encrypted with a different key."}
    end
  end
  ...
  ```elixir
  %{"hola": " qué tal ｸｿ"}
  |> Fulib.Cipher.cipher  # "qW0Voj3h4nglx4NPy8aLXVY5ze5V3OBu5IoaQTMUUbU%3D"
  |> Fulib.Cipher.parse  #  {:ok, %{"hola" => " qué tal ｸｿ"}}
  ```
  """
  def cipher(data), do: data |> Jason.encode!() |> encrypt
  ...
  def parse(crypted) do
    case crypted |> decrypt do
      ...
      decrypted -> Jason.decode(decrypted)
    end
  end
  ...
  def sign(url, base, payload) when is_map(payload) do
    ...
    signature = :crypto.hash(:md5, base) |> Fulib.Cipher.Digest.hexdigest()
    {_, _, micros} = :os.timestamp()
    # 8 characters long
    pepper = micros |> Integer.to_string() |> String.pad_leading(8)
  ...
  @doc """
    ...
    ```elixir
    "/bla/bla?p1=1&p2=2"
    |> Fulib.Cipher.sign_url  # "/bla/bla?p1=1&p2=2&signature=4B6WOiuD9N39K7p%2BnqNIljGh5F%2F%2BnHRQGZC9ih%2Bh%2BHGZc8Tz0KdRJXC%2B5M%2B8%2BHZ2mAXPh3jQcSRieTq4dGm5Ng%3D%3D"
    ```
  """
  ...
  defp validate_denied(denied, [r | rest]) do
    ...
    case n in denied do
      true ->
        {:error,
         "Parameter '#{r}' is not allowed by given signature. Denials: #{inspect(denied)}"}

      false ->
        validate_denied(denied, rest)
    end
  end
  ...
  defp validate_base(parsed, base) when is_binary(base) do
    signature =
      ...
      |> Fulib.Cipher.Digest.hexdigest()

    # removing pepper from parsed
    read_signature = String.slice(parsed["md5"], 0..-9)
  ...
  @doc """
    ...
    |> Fulib.Cipher.validate_signed_url  # {:ok, %{"md5" => "86e359da7ab4886f3525ac2b9c5edc5b  613146"}}
    ```
