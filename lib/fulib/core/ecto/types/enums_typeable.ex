defmodule Fulib.Ecto.EnumsTypeable do
  @moduledoc """
  数据类型的enum_type

  ## 参考资料

  1. https://www.postgresql.org/docs/9.4/arrays.html
  1. https://www.postgresql.org/docs/9.4/functions-array.html
  """
  defmacro __using__(opts \\ []) do
    quote do
      import Fulib.Model.Extends

      defstruct __enums_typeable__: true
          ...
          def cast([value | values]) do
            case {cast(value), cast(values)} do
              {{:ok, values1}, {:ok, values2}} ->
                {:ok, Fulib.compact(values1 ++ values2)}
