defmodule Fulib.Params.Def do
  ...
  defmacro defparams({name, _, [schema]}, do: block) do
    ...
    quote bind_quoted: [name: name, schema: schema, block: block] do
      module_name = Fulib.Params.Def.module_concat(Fulib.Params, __MODULE__, name)
  ...
  defmacro defschema(schema) do
    quote bind_quoted: [schema: schema] do
      normalized_schema = Fulib.Params.Def.normalize_schema(schema, __MODULE__)
      Module.eval_quoted(__MODULE__, Fulib.Params.Def.gen_root_schema(normalized_schema))

      normalized_schema
      |> Fulib.Params.Def.build_nested_schemas()
    ...
    acc =
      if embedded do
        ...
        module_def = {
          ...
          Fulib.Params.Def.gen_root_schema(sub_schema)
        }
  ...
  defp field_names(schema, filter) do
    for field <- schema, apply(filter, [field]), do: Keyword.get(field, :name)
  end
