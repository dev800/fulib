defmodule Fulib.Size do
  ...
  @type humanize_output ::
          {number, String.t()} | %{value: number, symbol: String.t()} | String.t()
  ...
  def humanize!(size, options \\ []) when is_integer(size) do
    ...
    symbols =
      Keyword.get_lazy(options, :symbols, fn ->
  ...
  defp exponent(size, ceil) do
    ...
    if exponent > @exponent_limit do
      raise SizeTooBigError,
        message: "`#{size}` size argument is out of the limits to be humanized"
    else
      exponent
    end
  end
